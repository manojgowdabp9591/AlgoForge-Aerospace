...
import { Shield, Activity, Target, CheckCircle2, MapPin, Briefcase, Cpu, Radio, Code, GitBranch } from "lucide-react";
import { motion } from "framer-motion";

export default function FlightSoftwareInternPage() {
  return (
    <PageLayout
      title={<>Internship: <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Embedded Flight Software</span></>}
      subtitle="Write the C++ Finite State Machine that flies, aborts, and recovers our launch vehicles."
    >
        ...
        <div className="lg:col-span-8 space-y-16">
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative bg-white/[0.02] border border-white/10 p-8 md:p-10 rounded-3xl backdrop-blur-md overflow-hidden group">
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-3 mb-6">
                    <div className="p-2 bg-cyan-500/10 rounded-lg text-cyan-400 border border-cyan-500/20"><Target size={24} /></div>
                    <h3 className="text-xl font-bold text-white tracking-wide">The Academic Project</h3>
                </div>
                <p className="text-white/70 leading-relaxed text-lg font-light">
                    As an Embedded Flight Software Intern at <strong className="text-white font-medium">Algoforge Aerospace</strong>, you will be handed the keys to Node 2—our ESP32-based onboard flight computer. 
                    <br/><br/>
                    You will write the strict C++ logic that fuses MPU6050 and BMP280 sensor data, calculates live vertical velocity, triggers parachute deployment at exact apogee, and beams JSON telemetry back to Earth via LoRa radio.
                </p>
            </div>
          </motion.section>
          ...
          <section className="space-y-8">
              ...
              <div className="grid gap-4">
                 <ListItem text="Develop a robust, non-blocking Finite State Machine (FSM) in C++ to control the rocket from Pad Idle to Touchdown." delay={0.1} />
                 <ListItem text="Implement calculus-based sensor fusion (integrating Z-axis acceleration and barometric pressure) to calculate live velocity." delay={0.2} />
                 <ListItem text="Write hardware drivers for the RA-02 LoRa transceiver to stream telemetry without bottlenecking the main loop." delay={0.3} />
                 <ListItem text="Program servo actuation logic for dual-deployment recovery systems (Drogue at Apogee, Main at 100m AGL)." delay={0.4} />
              </div>
          </section>
          ...
          <section className="space-y-8">
              ...
              <div className="grid gap-4">
                 <ListItem text="Pursuing a B.S. or M.S. in Computer Science, Computer Engineering, or Electrical Engineering." delay={0.1} />
                 <ListItem text="Demonstrated portfolio showcasing C++ projects on microcontrollers (ESP32, STM32, Arduino)." delay={0.2} />
                 <ListItem text="Strong grasp of basic calculus and physics (kinematics) to understand rocket flight dynamics." delay={0.3} />
                 <ListItem text="Understanding of hardware communication protocols (I2C, SPI, UART)." delay={0.4} />
              </div>
          </section>
          ...
          <section>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                 <PerkItem icon={Code} title="Flight Heritage" text="The code you write during this internship will literally fly on our hardware." />
                 <PerkItem icon={Cpu} title="Hardware Mentorship" text="Learn how to write bare-metal code that survives massive G-forces." />
                 <PerkItem icon={GitBranch} title="Portfolio Building" text="Graduate with a resume bullet that top-tier aerospace MNCs look for." />
                 <PerkItem icon={Radio} title="Full Context" text="Work directly with the founders building the Ground Station and UI." />
              </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="bg-black/40 border border-white/10 p-6 rounded-2xl relative overflow-hidden backdrop-blur-xl">
            ...
            <div className="space-y-6">
              ...
              <MetaRow icon={Cpu} label="Core Stack" value="C++ / ESP32 / SPI / I2C" />
            </div>
          ...
          <div className="sticky top-24"><Apply role="Embedded Flight Software Intern" /></div>
        </div>
      </div>
    </PageLayout>
  );
